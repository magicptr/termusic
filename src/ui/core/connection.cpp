...
/// Connection: the MPD server this application is a client of.
///
/// Everything that used to sit in the `MPD` module lives here, expressed as
/// setting items: two text fields, a numeric port, a switch, and the two
/// actions that talk to the server. The module only wires them to the
/// configuration; the shared list engine draws and drives them.
class ConnectionSection final : public SettingsListSection {
public:
...
protected:
  void fill(const CoreContext &context) override {
    ...
    items.push_back(note([this] {
      ...
      const bool connected = context_ != nullptr && context_->state.mpd_connected;
      return std::string("Server  ") + (connected ? "connected" : "disconnected");
    }));
    items.push_back(note([this] {
    ...
    items.push_back(note([this] {
      ...
      return "Using   " + effective.host + ":" + std::to_string(effective.port) +
             "  (" + effective.host_source + ")";
    }));
    items.push_back(heading("Server"));
...
std::unique_ptr<SettingsSection> makeConnectionSection() {
  return std::make_unique<ConnectionSection>();
}
